...
class MasterCmd(cmd.Cmd):
    ...
    def do_menu(self, line):
        ...
        print('\tenum\t\tEnumerate outstation data points.')
        print('\tunsol_off\tDisable unsolicited messages.')
        print('\tdisco\t\tSwitch lights to disco mode.')
        print('\tquit')
    ...
    def do_enum(self, line):
        """Perform a slow scan to enumerate all points on the target outstation"""
        self.application.slow_scan.Demand()
      
    def do_unsol_off(self, line):
        """Disable unsolicited messages. """
        headers = [opendnp3.Header().AllObjects(60, 2), opendnp3.Header().AllObjects(60, 3), opendnp3.Header().AllObjects(60, 4)]
        self.application.master.PerformFunction("disable unsolicited", opendnp3.FunctionCode.DISABLE_UNSOLICITED, headers, opendnp3.TaskConfig().Default())
    def do_disco(self, line):
        """Party time!!"""
        while True:
            index = random.randint(0,2)
            self.application.send_direct_operate_command(opendnp3. ControlRelayOutputBlock(opendnp3.ControlCode.LATCH_ON), index, command_callback)
            sleep(0.5)
            self.application.send_direct_operate_command(opendnp3. ControlRelayOutputBlock(opendnp3.ControlCode.LATCH_OFF), index, command_callback)
